...
class Life(object):
    ...
    def __init__(self,nrows,ncols,percentage):
        ...
        for i in range(self.nrows):
            ...
            self.boardOld.append([0]*self.ncols)
            self.neighbors.append([0]*self.ncols)
            self.grid.append([0]*self.ncols)
...
# Use modular arithmetic to code 2D pattern as 1D values.
    ...
    def printBoardGraphics(self,canvas,grid):
        """
...
# Draw in live cells
        ...
        for i in range(self.nrows):
            for j in range(self.ncols):
                if self.board[j][i]!=self.boardOld[j][i]:
                    self.boardOld[j][i] = self.board[j][i]
                    if self.board[j][i] == 1:
                        canvas.itemconfig(grid[j][i],fill='red')
                    else:
                        canvas.itemconfig(grid[j][i],fill='white')
    ...
    def update(self):
        ...
        for i in range(self.nrows):
            for j in range(self.ncols):
                self.boardOld[i][j]=self.board[i][j]
...
class Controller(object):
    ...
    def __init__(self):
        ...
        self.enumerateButton = Button(self.stillFrame, text='Find Still Lifes',command=self.findStillLife)
        ...
        for i in range(self.nrows):
            for j in range(self.ncols):
                self.grid[j][i]=self.canvas.create_rectangle(10*i+5,10*j+5,10*(i+1)+5,10*(j+1)+5,fill='white')
    ...
    def openFile(self):
        ...
        if file:
            ...
            self.pattern.printBoardGraphics(self.canvas,self.grid)
            self.canvas.update()
...
# the size of the board within a 1 square safety margin around the perimeter.
        ...
        n = (self.nrows-2)*(self.ncols-2)
        i = int(self.stillSize.get())
        for tuplej in combinations(range(n),i):
...
#Convert pattern tuple to format for board of dimension nrows*ncols
            list=[]
            for val in tuplej:
                i = 1 + val/(self.ncols-2)
                j = 1 + val%(self.ncols-2)
